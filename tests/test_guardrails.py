"""Offline tests of the Agent Core guardrails (fake model, no API key).
...
from tests.conftest import ToolCallingFakeModel, tool_call
...
from tools import mlops_tools
from tests.test_diagnostic_graph import alert_state
...
def test_step_limit_forces_a_partial_diagnosis(monkeypatch):
    ...
    # Chapter 3 agent: no checkpointer
...
def test_prometheus_result_starts_with_its_query_even_when_truncated(monkeypatch):
    promql = "100 - (avg(rate(node_cpu_seconds_total{mode=\"idle\"}[5m])) * 100)"
    # Aggregated query: the series has no label, only the query line tells what it is.
    body = {"status": "success", "data": {"result": [
        {"metric": {}, "values": [[i, "12.8"] for i in range(500)]}]}}
    monkeypatch.setenv("TOOL_OUTPUT_MAX_CHARS", "200")
    monkeypatch.setattr(mlops_tools.requests, "get", lambda *a, **k: _Resp(body))
    llm = recording_model(
        tool_call("PrometheusQuery", {"query": promql, "time_range_minutes": 5, "step_seconds": 30}),
...
def test_prometheus_error_also_names_its_query():
    result = mlops_tools.PrometheusQuery.invoke({"query": "up", "time_range_minutes": 5, "step_seconds": -5})
    assert result == "PromQL query: up\nAn unexpected error occurred: step_seconds must be positive."
